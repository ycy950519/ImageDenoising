...
activation: 激活函数
'''
conv1 = tf.layers.conv2d(inputs_, filters=32,kernel_size=(5, 5),strides=(1, 1),padding='SAME',use_bias=True,activation=tf.nn.relu )
conv1=tf.layers.conv2d(conv1,filters=64,kernel_size=(1,1),strides=(1,1),padding='same',use_bias=True,activation=tf.nn.relu)

conv2 = tf.layers.conv2d(inputs_, filters=64,kernel_size=(3,3),strides=(1, 1),padding='SAME',use_bias=True,activation=tf.nn.relu )
conv3 = tf.layers.conv2d(inputs_, filters=64,kernel_size=(1,1),strides=(1, 1),padding='SAME',use_bias=True,activation=tf.nn.relu)

avepool=tf.layers.average_pooling2d(inputs_,pool_size=(3,3),strides=1,padding='SAME')
...
#使用高斯噪声进行去噪
train_sp_data(gaussian_data, 'graphs', 'gaussian', 500) #(9, 256, 256, 1)
